  ...
  return (
    <div>
      <div className="fixed top-6 left-0 w-full z-50 flex justify-center px-4">
        ...
        <header className="bg-[#18181b] border border-zinc-800/50 rounded-full h-20 py-2 px-6 flex items-center justify-between w-full max-w-6xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] backdrop-blur-md relative">
          ...
          <nav className="hidden lg:flex items-center space-x-1 relative">
            {navItems.map((item) => (
              <a
                key={item.name}
                href={item.href}
                className="relative px-3.5 py-2 rounded-full text-xs xl:text-sm font-medium text-white hover:text-gray-900 transition-colors duration-300 whitespace-nowrap"
        ...
        {isOpen && (
          <div className="absolute top-24 left-4 right-4 bg-[#18181b] border border-zinc-800/80 rounded-2xl p-6 shadow-2xl lg:hidden animate-fade-in z-50">
