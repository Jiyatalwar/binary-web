...
export default function Navbar() {
  ...
  // Unified absolute anchor paths across your ecosystem
  const navItems = [
      ...
      <div className="fixed top-4 sm:top-6 left-0 w-full z-50 flex justify-center px-4 sm:px-6 lg:px-8">
        ...
        <header className="bg-[#18181b]/70 rounded-full h-20 py-2 px-4 sm:px-6 lg:px-8 flex items-center justify-between w-full max-w-7xl backdrop-blur-md relative border border-zinc-800/40">
          ...
          <nav className="hidden lg:flex items-center space-x-1 relative">
            {navItems.map((item) => {
              const isHovered = activeTab === item.name;
              return (
                <a
                  key={item.name}
                  href={item.href}
                  className={`relative px-3 py-2 rounded-full text-[11px] xl:text-xs 2xl:text-sm font-medium transition-colors duration-300 whitespace-nowrap z-10 ${
                    isHovered ? 'text-zinc-950' : 'text-white'
                  }`}
            ...
            <button
              ...
              className={`relative p-2 ml-1 xl:ml-2 rounded-full transition-colors duration-300 focus:outline-none z-10 ${
                activeTab === 'search-icon' ? 'text-zinc-950' : 'text-white'
              }`}
