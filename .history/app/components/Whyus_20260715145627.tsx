...
const Whyus = () => {
  ...
  return (
    <>
      {/* Enhanced native keyframes for the deliberate 5-second reveal flow with a stagger hook */}
      <style>{`
        ...
        .animate-present-slow {
          ...
          opacity: 0; /* Keeps cards hidden until animation delay starts */
        }
      `}</style>
      ...
      <section className="mt-[-80px] relative py-25 overflow-hidden bg-cover bg-center bg-no-repeat bg-fixed" style={{ backgroundImage: "url('https://img.magnific.com/free-photo/gradient-blue-abstract-background-smooth-dark-blue-with-black-vignette-studio_1258-67827.jpg?semt=ais_hybrid&w=740&q=80')" }}>
        <div className="w-full max-w-6xl mx-auto px-4 lg:px-0">
          ...
          {/* Bottom Section: 3-Column Features Grid with One-by-One Cascade Animation */}
          <div className="mt-[-80px] grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-x-10 lg:gap-y-10 border-t border-white pt-16 w-full max-w-6xl mx-auto">
            {features.map((item, index) => (
              <div 
                key={index} 
                className="flex flex-col items-start group p-6 rounded-2xl bg-transparent hover:bg-white transition-all duration-300 cursor-pointer shadow-sm hover:shadow-2xl overflow-hidden animate-present-slow"
                // 💡 Staggers initialization time by an incremental fraction of a second per item index
                style={{ animationDelay: `${0.8 + index * 0.25}s` }}
              >
