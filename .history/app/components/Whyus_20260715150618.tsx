        ...
        <div className="w-full max-w-6xl mx-auto px-4 lg:px-0">
          ...
          <div className="mt-[-80px] grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-x-10 lg:gap-y-10 border-t border-white pt-16 w-full max-w-6xl mx-auto">
            {features.map((item, index) => (
              <div 
                key={index} 
                className="flex flex-col items-start group p-6 rounded-2xl border border-white/10 bg-gradient-to-br from-white/10 to-transparent hover:bg-none hover:bg-white transition-all duration-500 cursor-pointer  hover:shadow-2xl overflow-hidden animate-present-slow"
                // 💡 Starts after the hero section is visible, then cascades each card precisely 2 seconds apart
                style={{ animationDelay: `${1 + index * 2}s` }}
              >
