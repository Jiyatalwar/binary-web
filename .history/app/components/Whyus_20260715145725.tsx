...
const Whyus = () => {
  ...
  return (
    <>
      {/* CSS Keyframes handling the smooth slow entrance drop logic */}
      <style>{`
        ...
        .animate-present-slow {
          animation: slowPresentation 2s cubic-bezier(0.16, 1, 0.3, 1) forwards;
          opacity: 0; /* Ensures visibility holds at 0 until the specific index timer kicks in */
        }
      `}</style>
      ...
      <section className="mt-[-80px] relative py-25 overflow-hidden bg-cover bg-center bg-no-repeat bg-fixed" style={{ backgroundImage: "url('https://img.magnific.com/free-photo/gradient-blue-abstract-background-smooth-dark-blue-with-black-vignette-studio_1258-67827.jpg?semt=ais_hybrid&w=740&q=80')" }}>
        <div className="w-full max-w-6xl mx-auto px-4 lg:px-0">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-20 items-start mb-24">
            ...
            <div className="flex flex-col items-start justify-center h-full pt-2 lg:pt-8 group/desc animate-present-slow" style={{ animationDelay: '1.5s' }}>
          ...
          {/* Bottom Section: 3-Column Features Grid with One-by-One Deliberate Sequence */}
          <div className="mt-[-80px] grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-x-10 lg:gap-y-10 border-t border-white pt-16 w-full max-w-6xl mx-auto">
            {features.map((item, index) => (
              <div 
                ...
                // 💡 Each subsequent card waits exactly 5 additional seconds to animate into view
                style={{ animationDelay: `${3.5 + index * 5}s` }}
              >
