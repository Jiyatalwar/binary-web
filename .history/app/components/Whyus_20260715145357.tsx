...
const Whyus = () => {
  ...
  return (
    <>
      {/* Native keyframe styles to orchestrate the deliberate 5-second reveal entrance flow */}
      <style>{`
        ...
        .animate-present-slow {
          animation: slowPresentation 5s cubic-bezier(0.16, 1, 0.3, 1) forwards;
        }
      `}</style>
      ...
      <section className="mt-[-80px] relative py-25 overflow-hidden bg-cover bg-center bg-no-repeat bg-fixed" style={{ backgroundImage: "url('https://img.magnific.com/free-photo/gradient-blue-abstract-background-smooth-dark-blue-with-black-vignette-studio_1258-67827.jpg?semt=ais_hybrid&w=740&q=80')" }}>
        <div className="w-full max-w-6xl mx-auto px-4 lg:px-0">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-20 items-start mb-24">
            ...
            {/* Left Column: Hero Text with Direct Interaction Group Wrapper */}
            <div className="group/hero cursor-default select-none animate-present-slow">
              ...
              {/* Hero text zooms outward smoothly anchored cleanly to the left edge */}
            ...
            {/* Right Column: Narrative Info Text & Primary Button */}
            <div className="flex flex-col items-start justify-center h-full pt-2 lg:pt-8 group/desc animate-present-slow [animation-delay:0.4s]">
              ...
              <button className="border border-white text-white hover:bg-white hover:text-zinc-950 px-8 py-2.5 text-xs tracking-widest uppercase font-semibold transition-all duration-300 hover:scale-105 active:scale-95 transform">
                Button
              </button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-x-10 lg:gap-y-10 border-t border-white pt-16 w-full max-w-6xl mx-auto animate-present-slow [animation-delay:0.8s]">
