...
const Solutions = () => {
  ...
  {/* Rounded capsule container that mirrors the reference navigation design */}
  <div className="flex flex-wrap gap-2 items-center justify-center bg-zinc-900 text-white p-2 rounded-full max-w-fit mx-auto shadow-md"> 
    
    {/* --- CATEGORY: SMART INFRASTRUCTURE --- */}
    <button 
      onClick={() => setSelectedCategory('SmartInfrastructure')}
      className={`px-6 py-2.5 text-sm md:text-base font-semibold rounded-full transition-all duration-300 focus:outline-none ${
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-6 transition-all duration-500">
          {categoryData[selectedCategory].cards.map((card, index) => {
            ...
            return (
              <div 
                key={index}
                className={`group flex flex-col justify-between items-center text-center bg-gradient-to-b from-transprent via-[#162234]/10 to-[#082567]/20 border border-[#04043C]/20 rounded-[2rem] p-8 relative overflow-hidden hover:-translate-y-1 transition-all duration-300 ${gridClasses}`}
              >
