...
const SolutionsContent = () => {
  ...
  const solutionsDataRepository: Record<string, SolutionData> = {
    ...
    'Application Service': {
      ...
      components: [
        ...
        {
          id: 81, // Fixed duplicate ID conflict
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-20 text-gray-900 min-h-screen">
      
      {/* Top Level Header */}
      <div className="mb-8 md:mb-12">
        <span className="text-gray-500 font-semibold text-xs uppercase tracking-widest block mb-2.5">
          Our Solutions
        </span>
        <h2 className="text-3xl sm:text-4xl md:text-5xl font-black tracking-tight leading-[1.15] bg-gradient-to-br from-[#004785] via-[#002142] to-[#000d1d] bg-clip-text text-transparent">
          Elevate Your Business with Innovative IT System Integration by Binary Global.
        </h2>
      </div>

      {/* Solutions Navigation Selector Tabs - Native horizontal scroll experience on Mobile */}
      <div className="relative mb-10 md:mb-12">
        <div className="flex overflow-x-auto pb-3 gap-2.5 scrollbar-none snap-x snap-mandatory md:flex-wrap md:overflow-x-visible md:pb-0">
          {solutionsNavbar.map((solutionName) => {
            ...
            return (
              <Link
                ...
                href={href}
                scroll={false}
                className={`px-5 py-2.5 rounded-full text-xs sm:text-sm font-medium transition-all duration-200 border whitespace-nowrap snap-center shrink-0 shadow-sm active:scale-95 md:hover:scale-[1.02] ${
                  isActive
      ...
      <div className="mb-10 md:mb-12 border-l-4 border-[#004785] pl-4 md:pl-6">
        <span className="text-xs font-bold uppercase tracking-widest block mb-2 text-[#004785]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
        {currentSolution.components.map((component) => (
          <div 
            key={component.id}  
            className="group flex flex-col justify-between rounded-2xl bg-white border border-gray-200/70 shadow-sm hover:border-transparent transition-all duration-300 overflow-hidden hover:shadow-xl h-full md:hover:-translate-y-1"
          >
            <div className="flex flex-col">
              ...
              {/* Text Info Container */}
              <div className="p-5 md:p-6 flex flex-col space-y-2.5">
                <h4 className="text-base font-bold text-gray-900 tracking-tight leading-snug group-hover:text-[#004785] transition-colors duration-200">
                  {component.title}
                </h4>
      ...
      {currentSolution.components.length === 0 && (
        <div className="text-center py-16 border border-dashed border-gray-200 rounded-2xl bg-white shadow-sm">
...
const Solutions = () => {
  return (
    <Suspense fallback={<div className="text-center py-20 text-sm text-gray-500">Loading Solutions Hub Architecture...</div>}>
      <SolutionsContent />
    </Suspense>
  );
};
