...
const Whyus = () => {
  const features = [
    {
      icon: <Layers size={36} strokeWidth={1.2} />,
    ...
    {
      icon: <Target size={36} strokeWidth={1.2} />,
  ...
  return (
    <>
      ...
      <section className="relative w-full overflow-hidden bg-[#020617] flex flex-col justify-center min-h-[700px] lg:h-[780px] lg:max-h-[800px] py-12 md:py-16 mt-[-60px]">
        ...
        <div className="relative z-10 w-full max-w-6xl mx-auto px-6 lg:px-4 flex flex-col justify-between h-full gap-8 lg:gap-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-5 border-t border-white/10 pt-6 lg:pt-8 w-full">
            {features.map((item, index) => (
              <div 
                key={index} 
                className="group flex flex-col items-start p-5 rounded-xl border border-white/5 bg-gradient-to-br from-white/[0.04] to-transparent backdrop-blur-sm transition-all duration-300 cursor-pointer shadow-sm hover:bg-white hover:border-white hover:shadow-2xl overflow-hidden animate-present-slow"
                style={{ animationDelay: `${0.5 + index * 0.1}s` }}
              >
                {/* Vector Accent Node */}
                <div className="mb-3 text-white group-hover:text-cyan-600 transition-colors duration-300">
                  {item.icon}
                </div>
                
                {/* Feature Header Segment */}
                <h3 className="text-xs sm:text-sm font-bold tracking-wider uppercase mb-1.5 text-cyan-400 group-hover:text-cyan-600 leading-tight transition-colors duration-300">
                  {item.titleLine1} 
                  <span className="font-semibold text-white group-hover:text-black block transition-colors duration-300">
                    {item.titleLine2}
                  </span>
                </h3>
                
                {/* Description Body Block */}
                <p className="text-zinc-400 group-hover:text-zinc-600 text-[11px] sm:text-xs leading-normal transition-colors duration-300">
                  {item.desc}
                </p>
              </div>
            ))}
