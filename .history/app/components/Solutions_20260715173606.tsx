...
const Solutions = () => {
  ...
  return (
    /* Adjusted baseline py-16 instead of arbitrary offsets to give predictable height */
    <section id="solutions" className="relative py-16 md:py-24 overflow-hidden bg-cover bg-center bg-no-repeat bg-fixed" style={{ backgroundImage: "url('')" }}>
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        {/* --- HERO / HEADING --- */}
        {/* Cleaned negative margins that were pulling content out of context boundaries */}
        <div className="max-w-4xl mx-auto text-center flex flex-col items-center mb-12">
          <h2 className="text-5xl md:text-7xl font-extrabold tracking-tight mb-4 leading-[1.1] bg-gradient-to-b from-[#07065B] via-[#090979] to-[#020024] bg-clip-text text-transparent">
              Our Solutions
          </h2>
          <p className="text-gray-900/60 text-sm md:text-xl max-w-2xl font-medium leading-relaxed">
        ...
        <div className="border-t border-zinc-200/80 pt-8 mb-12">
          <div className="flex flex-wrap gap-x-8 gap-y-4 items-center justify-center"> 
            
            <button 
              onClick={() => setSelectedCategory('SmartInfrastructure')}
              className={`text-lg md:text-xl font-bold transition-all duration-300 focus:outline-none pb-1 ${
                selectedCategory === 'SmartInfrastructure' 
                  ? 'bg-gradient-to-r from-[#020024] to-[#090979] bg-clip-text text-transparent border-b-2 border-cyan-600' 
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-6 transition-all duration-500">
          {categoryData[selectedCategory].cards.map((card, index) => {
            const gridClasses = card.isLarge  
              ? 'col-span-2 md:col-span-3 min-h-[220px]' 
              : 'col-span-2 md:col-span-2 min-h-[220px]';

            return (
              <div 
                key={index}
                className={`group flex flex-col justify-between items-center text-center bg-gradient-to-b from-[#E6E6F1] to-transparent border border-[#04043C]/10 rounded-[2rem] p-8 relative overflow-hidden hover:shadow-[0_20px_40px_rgba(0,0,0,0.04)] hover:-translate-y-1 transition-all duration-300 ${gridClasses}`}
              >
                ...
                <div className="relative z-10 flex flex-col items-center w-full mb-6">
                  ...
                  <p className="text-gray-900 text-sm md:text-base leading-relaxed font-small max-w-sm">
                    {card.description}
                  </p>
                </div>

                {/* Graphic Icon Render centered at the bottom of each card */}
                <div className="relative z-10 flex justify-center items-center mt-auto w-full">
                  {card.isLarge ? (
                    ...
                    <div className={`relative flex items-center justify-center w-28 h-16 rounded-2xl bg-slate-50 border border-slate-100 shadow-[0_8px_20px_rgba(0,0,0,0.03)] transition-all duration-300 
