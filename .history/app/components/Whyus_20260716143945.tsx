...
const Whyus = () => {
  ...
  return (
    <>
      ...
      {/* Main Section */}
      <section className="relative min-h-screen w-full overflow-hidden bg-[#020617] flex flex-col justify-center py-20 mt-[-60px]">
        ...
        {/* Content wrapper */}
        <div className="relative z-10 w-full max-w-6xl mx-auto px-6 lg:px-0 flex flex-col justify-between h-full gap-16">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-20 items-start">
            ...
            <div className="group/hero cursor-default select-none animate-present-slow" style={{ animationDelay: '0s' }}>
              <h1 className="text-4xl sm:text-5xl lg:text-7xl font-black text-gray-100 leading-tight tracking-tight origin-left transition-transform duration-700 ease-out lg:group-hover/hero:scale-105">
            ...
            <div className="flex flex-col items-start justify-center h-full group/desc animate-present-slow" style={{ animationDelay: '0.6s' }}>
              <p className="text-zinc-300 lg:text-white text-[18px] leading-relaxed max-w-xl origin-left transition-transform duration-700 ease-out lg:group-hover/desc:scale-[1.02]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 border-t border-white/20 pt-10">
            {features.map((item, index) => (
              <div 
                key={index} 
                className="group flex flex-col items-start p-6 rounded-2xl border border-white/10 bg-gradient-to-br from-white/10 to-transparent transition-all duration-500 cursor-pointer shadow-sm hover:bg-white hover:border-white hover:shadow-2xl overflow-hidden animate-present-slow"
                style={{ animationDelay: `${1.2 + index * 0.15}s` }}
              >
                {/* Icon Container */}
                <div className="mb-4 text-white group-hover:text-cyan-600 lg:group-hover:scale-110 transition-all duration-300 origin-left">
                  {item.icon}
                </div>
                
                {/* Feature Title */}
                <h3 className="text-sm sm:text-base font-bold tracking-widest uppercase mb-2 text-cyan-300 group-hover:text-cyan-600 leading-tight transition-all duration-300 lg:group-hover:scale-105 origin-left">
                  {item.titleLine1} 
                  <span className="font-normal text-white group-hover:text-zinc-950 block">
                    {item.titleLine2}
                  </span>
                </h3>
