...
const Solutions = () => {
  ...
  return (
    <section 
      id="solutions" 
      className="relative h-[700px] w-full overflow-hidden bg-cover bg-center bg-no-repeat bg-fixed flex flex-col items-center justify-center py-6 px-4" 
    ...
    >
      ...
      <div className="max-w-6xl w-full mx-auto px-4 flex flex-col h-full justify-between">
        ...
        {/* --- HERO / HEADING --- */}
        <div className="max-w-4xl mx-auto text-center flex flex-col items-center mt-2">
          <h2 className="mb-1 text-3xl md:text-5xl font-extrabold tracking-tight leading-tight bg-gradient-to-b from-[#07065B] via-[#090979] to-[#020024] bg-clip-text text-transparent">
              Our Solutions
          </h2>
          <p className="text-gray-800 tracking-normal text-xs md:text-base max-w-xl font-normal leading-relaxed">
        ...
        <div className="border-t border-zinc-200/80 pt-3 pb-2 my-2">
        ...
        {/* --- BENTO GRID CARDS --- */}
        <div className="grid grid-cols-2 md:grid-cols-6 gap-4 transition-all duration-500 flex-1 items-stretch mb-2">
          {categoryData[selectedCategory].cards.map((card, index) => {
            ...
            return (
              <div 
                key={index}
                className={`group flex flex-col justify-between items-center text-center bg-gradient-to-b from-[#E6E6F1] to-transparent border border-[#04043C]/20 rounded-[1.5rem] p-5 relative overflow-hidden hover:shadow-[0_12px_24px_rgba(0,0,0,0.03)] hover:-translate-y-0.5 transition-all duration-300 ${gridClasses}`}
              >
                ...
                {card.graphicType === "rings" && (
                  <>
                    <div className="absolute -bottom-8 -right-8 w-36 h-36 border border-zinc-100 rounded-full opacity-40 pointer-events-none" />
                    <div className="absolute -bottom-12 -right-12 w-48 h-48 border border-zinc-100 rounded-full opacity-40 pointer-events-none" />
                  </>
                )}

                {/* Centered Card Text Content */}
                <div className="relative z-10 flex flex-col items-center w-full mb-2">
                  <h3 className="font-extrabold text-sm md:text-base mb-1.5 bg-gradient-to-b from-[#07065B] via-[#090979] to-[#020024] bg-clip-text text-transparent group-hover:text-black transition-colors line-clamp-1">
                    {card.title}
                  </h3>
                ...
                <div className="relative z-10 flex justify-center items-center mt-auto">
                  {card.isLarge ? (
                    ...
                    <div className="relative flex items-center justify-center w-16 h-16 rounded-full bg-slate-50 border border-slate-100 shadow-[0_6px_14px_rgba(0,0,0,0.03)] group-hover:scale-105 transition-transform duration-300">
                      {card.icon}
                      <span className="absolute -top-0.5 w-1.5 h-1.5 bg-blue-500 rounded-full animate-ping" />
                      <span className="absolute -left-3 w-1 h-1 bg-indigo-400 rounded-full" />
                      <span className="absolute -right-4 w-1 h-1 bg-indigo-400 rounded-full" />
                    </div>
                  ) : (
                    // Small Card Graphic Treatment
                    <div className={`relative flex items-center justify-center w-24 h-14 rounded-xl bg-slate-50 border border-slate-100 shadow-[0_6px_14px_rgba(0,0,0,0.02)] transition-all duration-300 
