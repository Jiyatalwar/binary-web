...
const SolutionsContent = () => {
  ...
  // Automatically pulls the viewport down cleanly when navbar links fire
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-start gap-2 sm:gap-3 my-[30px] sm:my-[40px]">
        {solutionsNavbar.map((solutionName) => {
          const isActive = activeSolution.toLowerCase() === solutionName.toLowerCase();
          
          // Preserves hash target location on structural page state updating
          ...
          return (
            <Link
              key={solutionName}
              href={href}
              scroll={false} // Prevents next.js framework wrapper from instantly resetting viewport to top of window
              className={`px-4 sm:px-6 py-2 sm:py-2.5 rounded-full text-[11px] sm:text-xs md:text-sm font-medium shadow-sm border transition-all duration-300 hover:scale-[1.03] origin-left cursor-pointer whitespace-nowrap text-center ${
                isActive
